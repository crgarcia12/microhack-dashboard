...
### .NET Conventions (Backend — src/api/)

- **Minimal API style**: Define endpoints in `Program.cs` or via `MapGroup()` extension methods — no MVC controllers
- **Async/await**: All I/O-bound operations must be async. Use `Task<T>` return types with `Async` suffix
- **Nullable reference types**: Enabled project-wide — use `?` for nullable, assert non-null with `!` only when guaranteed
- **Dependency Injection**: Register services via `builder.Services` in `Program.cs`. Prefer interfaces for testability
- **Result types**: Return `Results.Ok()`, `Results.NotFound()`, `Results.BadRequest()` — not raw status codes
- **Configuration**: Use `IOptions<T>` pattern with strongly-typed settings classes
- **Error handling**: Use middleware for global exception handling; return `ProblemDetails` for API errors
- **Naming**: PascalCase for types/methods/properties, camelCase for local variables and parameters
...
- **xUnit**: Use `[Fact]` for single tests, `[Theory]` with `[InlineData]` for parameterized tests. Use `[Trait("Category", "...")]` for filtering
- **Reqnroll**: Use `[Binding]` class attribute, `[Given]`/`[When]`/`[Then]` step attributes. Scenarios auto-discovered from `.feature` files
- **FluentAssertions**: Prefer `.Should().Be()`, `.Should().NotBeNull()` over xUnit `Assert.*` methods
